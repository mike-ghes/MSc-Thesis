...
\begin{definition}
	...
	\begin{align}
		...
		= \bigoplus_{j=1}^k (-1)^j &\;
			\left(\;	
				\Big[\!\Big[ 	\left(\boldsymbol{a}^{[\![1,n]\!]_\mathbb{N}}\right), 
					\;\;\;
					\left(\boldsymbol{b}^{[\![1,i_j)\!)_\mathbb{N}} 
						\oplus \boldsymbol{a}^{\hset{i_j}}
						\oplus \boldsymbol{b}^{(\!(i_j,n]\!]_\mathbb{N}}\right) 
				\Big]\!\Big]_{\mathbb{R}^n} \right.\;
			\notag\\
			&\ominus \left.
				\Big[\!\Big[ 	\left(\boldsymbol{a}^{[\![1,i_j)\!)_\mathbb{N}}
						\oplus \boldsymbol{b}^{\hset{i_j}}
						\oplus \boldsymbol{a}^{(\!(i_j, n]\!]_\mathbb{N}}\right), 
					\;\;\;		 
					\left(\boldsymbol{b}^{[\![1,n]\!]_\mathbb{N}}\right) 			
				\Big]\!\Big]_{\mathbb{R}^n}
			\;\right)
	\end{align}
\end{definition}
...
The intervals $[\![1, i_j)\!)_\mathbb{N}$ and $(\!(i_j, n]\!]_\mathbb{N}$ are intervals over vector indices just as in Chapter 3.
Thus, the term $\boldsymbol{a}^{[\![1,i_j)\!)_\mathbb{N}}$ refers to the vector $(a_1, a_2, \ldots, a_{i_j-1})$ 
while the term $\boldsymbol{b}^{(\!(i_j,n]\!]_\mathbb{N}}$ refers to $(b_{i_j+1}, b_{i_j+2}, \ldots, b_{n})$.
...
\begin{equation*}
	[\![a_1, b_1]\!]_\mathbb{R}
	\times \ldots \times
	[\![a_{i_{j-1}}, b_{i_{j-1}}]\!]_\mathbb{R}
	\times
	[\![a_{i_j}, a_{i_j}]\!]_\mathbb{R}
	\times
	[\![a_{i_{j-1}}, b_{i_{j-1}}]\!]_\mathbb{R}
	\times \ldots \times
	[\![a_n, b_n]\!]_\mathbb{R}
...
Unlike the previous intervals which were over integers, each of these range continuously over $\mathbb{R}$.
...
The boundary of a $(k+1)$-rectangle was the sum $\oplus$, of $2(k+1)$, $k$-rectangles.
...
\begin{figure}[ht] 
	...
	\end{tikzpicture}
	\caption[Boundary of a boundary (of a (2-)rectagle)] {
		The boundary of rectangle gives a cycle of oriented edges. 
...
So each term representing a $(k-2)$-rectangle will occur twice in the sum.
