...
We will put off formal definitions until the next chapter.
}
...
\centering
\begin{tikzpicture}[scale=1.7, domain=0:5]
...
\todo[inline]{
- For an $n$-cube we can take $i_j \in \mathcal{P}[\![a_j,b_j]\!]$ for each side.

- $I_{i_1, \ldots, i_n} = i_1 \times \ldots \times i_n$ and some point $x_{i_1, \ldots, i_n}$ in $I_{i_1, \ldots, i_n}$

then $f_P = \sum_{i_1} \ldots \sum_{i_n} f(t_{i_1, \ldots, i_n}) \text{vol} (I_{i_1,\ldots,i_n})$

$\text{vol}[\![a,b]\!] = (b_1 - a_1) \cdot \ldots \cdot (b_n - a_n)$

(easily shown that $\text{vol}\ominus[\![a,b]\!] = - \text{vol}[\![a,b]\!]$)

volume of a $k$-cube in $\mathbb{R}^n$ with $k<n$ is 0.

- Recall discussion of refinements from Chapter 2.

- We can induce a partial ordering on $\mathcal{P}[\![a,b]\!]$ by saying $P \leq P'$ if $P'$ is a refinement of $P$.

$\int_{[\![a,b]\!]} f(x) \; dx = f_{\sup \{ \mathcal{P} [\![a,b]\!] \}}$ where P is $\sup \{ \mathcal{P} [\![a,b]\!] \}$.
}
...
\section{Stokes' Theorem on $n$-cubes}