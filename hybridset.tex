...
Non-compatible functions are not the end of the line.

Sometimes non-compatible functions will be desired

For non-compatible $f^A \in \mathbb{Z}^{U \to S}$ and $g^B \in \mathbb{Z}^{U \to S'}$, we tag $\oplus$ with an operation $* : S \times S' \to T$.
Then $f^A \oplus^* g^B$
...
It seems silly to have a partition for a single point

we could also use: 
\begin{equation}
	-1^{(-\infty, 0]} \oplus^+ 1^{[0, \infty)}
\end{equation}

Going from 3 regions to 2 is a small step, marked join allows us many gains in general

Given two piecewise functions with $n$ regions each.

Taking say the sum of such a function would lead to $n^2$ regions.
