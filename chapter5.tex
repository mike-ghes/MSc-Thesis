...
\todo[inline]{Overlap-Save vs. Overlap-Add}
...
\todo[inline]{``Replacement for step 2. in algorithm''}
...
From a theoretical perspective, very little is different between the continuous and discrete case.

Defined as:

\begin{equation}
	(F*G)[\tau] = \sum_{\tau=-\infty}^\infty F[\tau] \cdot G[t-\tau]
\end{equation}

Similarly, hybrid convolution for discrete functions is a simple swap:

\begin{equation}
	(f^{[a_f,b_f)} * g^{[a_g, b_g)} = adsfsad
\end{equation}

The one thing to note is that while $\int_{[a,b)} f(x)\; dx = \int_{[a,b]} f(x)\;dx$.
The same does not hold for summation; the openness or ``closed-ness'' of an interval matters.

\todo[inline]{Show correctness}
...
Hopefully this algorithm is very simple

Some strange looking hacks are required to ``make it work''

but the whole point is that hybrid convolution is a simpler framework with less case-based logic

Additionally, many of these steps, with the exception of deciding to leave \texttt{afbf} and \texttt{bfag} symbolic are 
entirely optional.
