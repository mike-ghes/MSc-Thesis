...
For the remainder of this chapter, we will only be interested in the space $\mathbb{N}_0 \times \mathbb{N}_0$ where 
...
\begin{equation}
	C_{i,j} = 	\left(\sum_{k \in K / last} A_{i,k}^{N_i \times M_k} B_{last,j}^{M_k \times P_j} \right)+ 
				\left(\sum_{k \in K' / last} A_{i,last}^{N_i \times M_k} B_{k,j}^{M_k \times P_j} \right)+ 
				A_{i,last}^{N_i \times M_{U\ominus all}} B_{last,j}^{M_{U \ominus all} \times P_j}
\end{equation}

\todo[inline]{Unfortunately these formulation for multiplication is not the easiest to work with.
The arithmetic used to convert any negatively oriented matrices to all positive ones is \emph{required} before the
equation can be interpreted.
Another approach would be to use:
$
	C_{i,j} = \sum_{[\![0,m)\!)} \R[\times] \left( 
		A_1^{M_1 \oplus M_2} \oplus A_2^{M_3} \oplus B_1^{M_1} \oplus B_2^{M_2 \oplus M_3}
	\right)
$}
