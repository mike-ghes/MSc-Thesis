...
\todo[inline]{``Rewrite with new example notation''}
...
We will suppose two block matrices
...
We will call these submatrices $A,B,C,D,E,F,G$ and $H$.

To partition these matrices we use $M,N,P$ as follows (indexing starting at zero):
\begin{align*}
	N_1 	&= [\![0, 1]\!] 	& N_2 	&= [\![2, 3]\!] 	\\
...
This will not change our process.
We can still write $Q$ as:
\begin{equation*}
...
\end{equation*}
The only difference is that when we add $(M_2 \oplus M_3)$, 
the set of indices we end up with is \emph{smaller} than the $M_3$ we started with.
Similarly, $R$ is also still written out as:
\begin{equation*}
	R = E^{(M_1 \oplus M_2) \times P_1} \oplus
		...
		H^{M_3 \times P_2}
\end{equation*}
Their product $S$ will be another $2\times 2$ block matrix:
\begin{equation*}
	S = Q \cdot R  =
...
We could, as before correct the negative orientation of $M_2$ algebraically, but this turns out to be unnecessary.
Recall that we are taking the approach that matrices are simply functions defined on $\mathbb{N} \times \mathbb{N}$.
When we select an axis of a matrix $M$ by way of $M|_{X=i}$ or $M|_{Y=j}$ we transform $M:X\times Y \to Z$
to a curried $M|_{X=i}:Y \to ( X \to Z)$ or $M|_{Y=j}:X \to (Y \to Z)$.
As this is a small example our curried functions only range over $\{ 0, 1, 2 \}$, so let's expand out each of our terms
as formal \emph{hybrid sets} (recall a hybrid function is a special hybrid set of ordered pairs):
\begin{align*}
	\sum_{m \in M ?} \R[\times] \bigg( 
		&\;\;\hset{
			(0 \mapsto \left[\begin{smallmatrix}\vphantom{b}a_1 \\ \vphantom{b}a_3 \end{smallmatrix}\right])^{+1}, \;
			(1 \mapsto \left[\begin{smallmatrix}\vphantom{b}a_2 \\ \vphantom{b}a_4 \end{smallmatrix}\right])^{+1}}  
		&&\oplus \hset{ 
			(0 \mapsto [ e_1 ])^{+1}, \;
			(1 \mapsto [e_\bot] )^{+1}} \\ 
		&\oplus \hset{ 
			(1 \mapsto \left[\begin{smallmatrix} b_\bot \\ b_\bot \end{smallmatrix}\right]^{-1} }
		&&\oplus \hset{ 
			(1 \mapsto [ e_\bot ] )^{-1} } \\
		&\oplus \hset{
			(1 \mapsto \left[ \begin{smallmatrix} b_\bot \\ b_\bot \end{smallmatrix} \right] )^{+1}, \;
			(2 \mapsto \left[ \begin{smallmatrix} b_1 \\ b_2 \end{smallmatrix}\right] )^{+1} }
		&&\oplus \hset{
			(2 \mapsto [ g_1 ] )^{+1},\; 
			(2 \mapsto [ g_2 ] )^{+1}}	\bigg)
\end{align*}
We are using $e_\bot$ and $b_\bot$ here to represent that the functions $E$ and $B$ are undefined for these points.
If we attempted to evaluate $Q$ at the points $(0,1)$ or $(1,1)$ we would find ourselves in the $A$ block not the $B$ block.
So, despite the implied existence of points at $B^{n \times 1}$ for points $n \in N_1$, this is out of the bounds of $B$.


Applying the $\times$-reduction, we group terms by their input value (e.g. $1 \mapsto x$ with $1 \mapsto y$) 
and multiply the outputs together (e.g. $1 \mapsto xy$) using the multiplicity to repeat or invert the $\times$ operator.
In this case, we are dealing only with multiplicities of $+1$ and $-1$ which correspond with multiplication and ``division''.
This is not true division, as $0 \times^{-1} 0 = 1$ but for non-zero values, it agrees with the normal understanding of
division.
Doing this yields:
\begin{align*}
...
After some cancellations in the second term, we evaluate $\times^{+1}$ as matrix multiplication and sum using normal
matrix addition:
\begin{equation*}
	{S_1}^{N_1 \times P_1} =
...
\end{equation*}
Computations for the other blocks are performed similarly.
